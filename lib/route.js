...
Route.prototype.middleware = function (fn) {
  ...
  switch (fn.length) {
    case 2: return function (err, ctx, next) { match(ctx) ? fn.call(ctx, err, next) : next(); };
    case 1: return function (ctx, next) { match(ctx) ? fn.call(ctx, next) : next(); };
    default: return function (ctx, next) { if (match(ctx)) fn.call(ctx); next(); };
  }
};